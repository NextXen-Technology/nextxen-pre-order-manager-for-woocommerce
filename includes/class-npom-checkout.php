...
class NPOM_Checkout {
	...
	/**
	 * This only applies to orders charged upon release.
	 *
	 * It changes the native WooCommerce thank you for order received message,
	 * including info about auto or manual payment and a release date, if one is set.
	 *
	 * @since 1.5.4
	 * @version 1.5.4
	 * @param string   $message WooCommerce native thank you order received message.
	 * @param WC_Order $order WooCommerce order object.
	 * @return string Updated message, with release date information, if applicable
	 */
	public function add_release_date_message( $message, $order ): string {
		// If there's no pre-order product, return the default WooCommerce thank message.
		$pre_order_product = NPOM_Order::get_pre_order_product( $order );
		if ( ! $pre_order_product ) {
			return $message;
		}

		// If the product isn't charged upon release, e.g. it's charged upfront, return the default WooCommerce thank you.
		$charge_upon_release = NPOM_Product::product_is_charged_upon_release( $pre_order_product );
		if ( ! $charge_upon_release ) {
			return $message;
		}

		$timestamp         = NPOM_Product::get_localized_availability_datetime_timestamp( $pre_order_product );
		$availability_date = NPOM_Product::get_localized_availability_date( $pre_order_product );

		/*
		 * Select message templates based on payment method - auto or manual payment.
		 *
		 * Use translatable message templates with/without placeholders instead of concatenating the date if it has been set.
		 * This allows translators to position the date naturally within each language's sentence structure,
		 * including languages with different word orders and semantic structures, e.g., Japanese, German, Arabic.
		 */
		if ( NPOM_Order::order_has_payment_token( $order ) ) {
			/*
			 * Message template for auto payment.
			 *
			 * Testing $timestamp instead of $availability_date ensures, so only concrete
			 * release dates are shown, as $availability_date might contain an empty or
			 * a string (e.g., "at a future date") when no specific release date is set.
			 */
			$message_template = $timestamp
				/* translators: %s is the availability date for the pre-order */
				? __( 'Your pre-order has been received. You will be automatically charged for your order via your selected payment method when your pre-order is released on %s. Your order details are shown below for your reference.', 'nextxen-pre-order-manager' )
				: __( 'Your pre-order has been received. You will be automatically charged for your order via your selected payment method when your pre-order is released. Your order details are shown below for your reference.', 'nextxen-pre-order-manager' );
		} else {
			// Message for manual payment upon release.
			$message_template = $timestamp
				/* translators: %s is the availability date for the pre-order */
				? __( 'Your pre-order has been received. You will be prompted for payment for your order when your pre-order is released on %s. Your order details are shown below for your reference.', 'nextxen-pre-order-manager' )
				: __( 'Your pre-order has been received. You will be prompted for payment for your order when your pre-order is released. Your order details are shown below for your reference.', 'nextxen-pre-order-manager' );
		}

		// Return the final message, formatted with the release date if available.
		return $timestamp ? sprintf( $message_template, esc_html( $availability_date ) ) : $message_template;
	}
	...
	/**
	 * Check if is a pre-order and charged up front.
	 *
	 * @return bool
	 */
	protected function is_pre_order_and_charged_up_front() {
		return NPOM_Cart::cart_contains_pre_order() && ! NPOM_Product::product_is_charged_upon_release( NPOM_Cart::get_pre_order_product() );
	}

	/**
	 * Conditionally remove any gateways that don't support pre-orders on the checkout page.
	 *
	 * When the pre-order is charged upon release, this is done because payment info is not
	 * required in this case so displaying gateways/payment fields is not needed. Instead the
	 * pay later gateway is displayed.
	 *
	 * When the pre-order is charged upfront, the cash on delivery gateway is removed as it
	 * is contradicts the upfront payment model.
	 *
	 * @since 1.0
	 *
	...
	public function maybe_remove_unsupported_gateways( $available_gateways ) {
		...
		if (
			...
			( $pay_page && $this->is_pre_order_and_charged_up_front() ) ||
			( is_checkout() && $this->is_pre_order_and_charged_up_front() )

		) {
			// Remove the cash on delivery gateway if the pre-order is charged upfront.
			unset( $available_gateways['cod'] );
		}

		return apply_filters( 'npom_remove_unsupported_gateways', $available_gateways, $this->is_pre_order_and_charged_upon_release() );
	}
	...
	public function modify_place_order_button_text( $default_text ) {
		...
		if ( ! NPOM_Cart::cart_contains_pre_order() ) {
			return $default_text;
		}

		// get custom text if set
		$text = get_option( 'npom_place_order_button_text' );
	...
	public function add_order_meta( $order_id ) {
		...
		// note the cart is checked here instead of the order since NPOM_Order::order_contains_pre_order() checks the meta that is about to be set here :)
		if ( ! NPOM_Cart::cart_contains_pre_order() ) {
			return;
		}

		// get pre-ordered product
		$product = NPOM_Cart::get_pre_order_product( $order_id );
		$order   = wc_get_order( $order_id );

		// indicate the order contains a pre-order
		$order->update_meta_data( '_npom_is_pre_order', 1 );

		// save when the pre-order amount was charged (either upfront or upon release)
		$order->update_meta_data( '_npom_when_charged', get_post_meta( $product->is_type( 'variation' ) ? $product->get_parent_id() : $product->get_id(), '_npom_when_to_charge', true ) );
	...
	public function update_payment_complete_order_status( $new_status, $order_id ) {
		...
		$order = wc_get_order( $order_id );
		if ( ! $order || ! NPOM_Order::order_contains_pre_order( $order ) ) {
			return $new_status;
		}

		$zero_cost_order = NPOM_Manager::is_zero_cost_order( $order );

		// Don't change status if pre-order will be charged upon release.
		if ( NPOM_Order::order_will_be_charged_upon_release( $order ) && ! $zero_cost_order ) {
			return $new_status;
		}
	...
	public function update_manual_payment_complete_order_status( $order_id ) {
		...
		$order_item = NPOM_Order::get_pre_order_item( $order );
		$product_id = $order_item ? wc_get_product( $order_item['product_id'] ) : null;

		// Failed order does not have "_npom_is_pre_order" meta key.
		...
		$is_failed_pre_order = in_array( current_action(), $action_hooks ) &&
			( NPOM_Order::get_pre_order_item( $order ) instanceof WC_Order_Item ) &&
			( $product_id && NPOM_Product::product_can_be_pre_ordered( $product_id ) );


		// don't update status for non pre-order orders
		if ( ! $is_failed_pre_order && ! NPOM_Order::order_contains_pre_order( $order ) ) {
			return;
		}
	...
	/**
	 * Removes the "_npom_is_pre_order" flag when charged ufront and the payment fails.
	 *
	...
	public function remove_is_pre_order_on_failure( $order_id ) {
		...
		// Don't update status for non pre-order orders.
		if ( ! NPOM_Order::order_contains_pre_order( $order ) ) {
			return;
		}
		...
		$order->delete_meta_data( '_npom_is_pre_order' );
		$order->save();
	}


} // end \NPOM_Checkout class
