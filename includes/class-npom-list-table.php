 ...
 */
class NPOM_List_Table extends WP_List_Table {
	 ...
	 * @since 1.0
	 * @return \NPOM_List_Table
	 */
	public function __construct() {

		parent::__construct(
			array(
				'singular' => __( 'Pre-order', 'nextxen-pre-order-manager' ),
				'plural'   => __( 'Pre-orders', 'nextxen-pre-order-manager' ),
				'ajax'     => false,
			)
		);
	}
	...
	public function get_views() {
		...
		if ( ! isset( $this->views ) ) {
			...
			if ( NPOM::is_hpos_enabled() ) {
				...
				$_cpt_clause    = 'AND _order.status != "trash"';

				$query = "
					SELECT COUNT(_order_meta.{$_order_meta_id}) AS count, _order_meta.meta_value AS status
					FROM {$_order_meta} _order_meta
					INNER JOIN {$_order} _order
						ON _order.{$_order_id} = _order_meta.{$_order_meta_id}
					WHERE _order_meta.meta_key = '_npom_status'
					{$_cpt_clause}
					AND EXISTS (
						SELECT 1 FROM {$_order_meta} _order_meta2
						WHERE _order_meta2.{$_order_meta_id} = _order_meta.{$_order_meta_id}
						AND _order_meta2.meta_key = '_npom_is_pre_order'
						AND _order_meta2.meta_value = '1'
					)
				...
				$query = "
					...
					AND _order_meta.{$_order_meta_id} IN (
						...
						WHERE meta_key = '_npom_is_pre_order'
			...
			$base_url = admin_url( 'admin.php?page=npom_manager' );
	...
	public function get_columns() {
		...
		$columns = array(
			...
			'status'            => '<span class="status_head tips" data-tip="' . wc_sanitize_tooltip( __( 'Status', 'nextxen-pre-order-manager' ) ) . '">' . __( 'Status', 'nextxen-pre-order-manager' ) . '</span>',
			'customer'          => __( 'Customer', 'nextxen-pre-order-manager' ),
			'product'           => __( 'Product', 'nextxen-pre-order-manager' ),
			'order'             => __( 'Order', 'nextxen-pre-order-manager' ),
			'order_status'      => __( 'Order status', 'nextxen-pre-order-manager' ),
			'order_date'        => __( 'Order date', 'nextxen-pre-order-manager' ),
			'availability_date' => __( 'Availability date', 'nextxen-pre-order-manager' ),
		);
		...
		switch ( $column_name ) {
			...
			case 'status':
				...
				if ( NPOM_Manager::can_pre_order_be_changed_to( 'cancelled', $order ) ) {
					...
					$actions['cancel'] = sprintf( '<a href="%s">%s</a>', esc_url( $cancel_url ), esc_html__( 'Cancel', 'nextxen-pre-order-manager' ) );
				}

				$column_content  = sprintf( '<mark class="%s tips" data-tip="%s">%s</mark>', NPOM_Order::get_pre_order_status( $order ), wc_sanitize_tooltip( NPOM_Order::get_pre_order_status_to_display( $order ) ), NPOM_Order::get_pre_order_status_to_display( $order ) );
			...
			case 'order':
				...
				$column_content = sprintf( '<a href="%s">%s</a>', $order->get_edit_order_url(), sprintf( __( 'Order %s', 'nextxen-pre-order-manager' ), $order->get_order_number() ) );
				break;
			...
			case 'availability_date':
				...
				$column_content = NPOM_Product::get_localized_availability_date( $product, '--' );
				break;
	...
	public function prepare_items() {
		...
		$per_page = $this->get_items_per_page( 'npom_edit_pre_orders_per_page' );
		...
		$args = apply_filters( 'npom_edit_pre_orders_request', $args );
		...
		if ( isset( $_GET['availability_date'] ) && absint( wp_unslash( $_GET['availability_date'] ) ) ) {
			...
			$products = $wpdb->get_col(
				$wpdb->prepare(
					...
					FROM {$wpdb->postmeta}
					WHERE meta_key = '_npom_availability_datetime'
			...
			if ( NPOM::is_hpos_enabled() ) {
				$search_ids_by_meta = $wpdb->get_col(
					$wpdb->prepare(
						...
						AND ( meta0.meta_key = '_npom_is_pre_order' AND meta0.meta_value = '1' )
						AND ( ( orders.billing_email LIKE %s )
						OR ( meta1.meta_key = '_npom_status' AND meta1.meta_value LIKE %s )
	...
	public function no_items() {
		...
		if ( isset( $_REQUEST['s'] ) ) : ?>
			<p><?php esc_html_e( 'No pre-orders found', 'nextxen-pre-order-manager' ); ?></p>
		<?php else : ?>
			<p><?php esc_html_e( 'Pre-orders will appear here for you to view and manage once purchased by a customer.', 'nextxen-pre-order-manager' ); ?></p>
			<p>
				<?php
					// translators: %1$s = Opening anchor tag for WooCommerce Pre-Order Manager documentation on WooCommerce.com, %2$s = closing anchor tag
					printf( esc_html__( '%1$sLearn more about managing pre-orders%2$s', 'nextxen-pre-order-manager' ), '<a href="https://nextxentech.com/docs/nextxen-pre-order-manager-for-woocommerce/" target="_blank">', ' &raquo;</a>' );
				?>
			</p>
			<p>
				<?php
					// translators: %1$s = Opening anchor tag for adding new product, %2$s = closing anchor tag
					printf( esc_html__( '%1$sSetup a product to allow pre-orders%2$s', 'nextxen-pre-order-manager' ), '<a href="' . esc_url( admin_url( 'post-new.php?post_type=product' ) ) . '">', ' &raquo;</a>' );
				?>
			</p>
			<?php
		endif;
	...
	public function extra_tablenav( $which ) {
		...
		if ( 'top' === $which ) {
			...
			<select id="dropdown_customers" style="width: 250px;" class="wc-customer-search" name="_customer_user" data-placeholder="<?php esc_attr_e( 'Search for a customer&hellip;', 'nextxen-pre-order-manager' ); ?>">
				<?php
				if ( ! empty( $_GET['_customer_user'] ) ) {
			...
			<select id="dropdown_products" class="wc-product-search" style="width: 250px;" name="_product" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'nextxen-pre-order-manager' ); ?>" data-action="woocommerce_json_search_products_and_variations">
				<?php
				if ( ! empty( $_GET['_product'] ) ) {
			...
			submit_button( esc_html__( 'Filter', 'nextxen-pre-order-manager' ), 'button', false, false, array( 'id' => 'post-query-submit' ) );
			echo '</div>';

			// Bulk action fields
			echo '<div id="bulk-action-fields" style="clear:left;padding-top:10px;display:none;">';
			echo '<textarea cols="62" rows="3" name="customer_message" placeholder="' . esc_html__( 'Optional message to include in the email to the customer', 'nextxen-pre-order-manager' ) . '"></textarea>';
			echo '</div>';
				...
				( function( $ ) {
					$( function() {
						...
						$( 'span.cancel' ).on( 'click', function( e ) {
							if ( ! window.confirm( '" . esc_js( __( 'Are you sure you want to cancel this pre-order?', 'nextxen-pre-order-manager' ) ) . "' ) ) {
								e.preventDefault();
							}
						});
					} );
			...
			$handle = 'npom-list-table-inline';
			wp_register_script( $handle, '', array( 'jquery' ), NPOM_VERSION, true );
			wp_enqueue_script( $handle );
			wp_add_inline_script( $handle, $javascript );
		} elseif ( 'bottom' === $which ) {
			// Bulk action fields
			echo '<div id="bulk-action-fields2" style="clear:left;padding-top:10px;display:none;">';
			echo '<textarea cols="62" rows="3" name="customer_message2" placeholder="' . esc_html__( 'Optional message to include in the email to the customer', 'nextxen-pre-order-manager' ) . '"></textarea>';
			echo '</div>';
		}
	}
	...
	private function render_availability_dates_dropdown() {
		...
		<select id="dropdown_availability_dates" name="availability_date" class="wc-enhanced-select" style="width: 250px;">
			<option<?php selected( $availability_date, 0 ); ?> value='0'><?php esc_html_e( 'Show all availability dates', 'nextxen-pre-order-manager' ); ?></option>
			<?php
			foreach ( $months as $arc_row ) {
				...
				printf(
					...
					esc_html( sprintf( __( '%1$s %2$d', 'nextxen-pre-order-manager' ), $wp_locale->get_month( $month ), $year ) )
				);
			}
			?>
		</select>
