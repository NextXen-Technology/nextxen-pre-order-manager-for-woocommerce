 ...
 * @since 2.1.0
 */
class NPOM_Product_Editor_Compatibility {
	/**
	 * Constructor
	 */
	...
	public function register_custom_blocks() {
		if ( isset( $_GET['page'] ) && 'wc-admin' === $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			BlockRegistry::get_instance()->register_block_type_from_metadata( NPOM_PLUGIN_PATH . '/build/admin/blocks/select-control' );
			BlockRegistry::get_instance()->register_block_type_from_metadata( NPOM_PLUGIN_PATH . '/build/admin/blocks/date-time-picker' );
			BlockRegistry::get_instance()->register_block_type_from_metadata( NPOM_PLUGIN_PATH . '/build/admin/blocks/message-control' );
		}
	}
	...
	public function add_meta_to_response( $response, $product ) {
		...
		$has_orders             = NPOM_Product::product_has_active_pre_orders( $product->get_id() );
		$availability_timestamp = NPOM_Product::get_localized_availability_datetime_timestamp( $product->get_id() );
		$availability_timestamp = esc_attr( ( 0 === $availability_timestamp ) ? '' : gmdate( 'Y-m-d H:i', $availability_timestamp ) );

		$data['pre_order_enabled']     = get_post_meta( $product->get_id(), '_npom_enabled', true );
	...
	public function save_custom_data_to_product_metadata( $product, $request ) {
		...
		if ( ! empty( $datetime ) ) {
			NPOM_Admin_Products::save_availability_date_time( $product_id, $datetime );
		} elseif ( ! is_null( $datetime ) ) {
			delete_post_meta( $product_id, '_npom_availability_datetime' );
		}

		if ( $is_enabled ) {
			update_post_meta( $product_id, '_npom_enabled', 'yes' );
		} elseif ( ! is_null( $is_enabled ) && false === $is_enabled ) {
			update_post_meta( $product_id, '_npom_enabled', '' );
		}
	}
	...
	public function add_pre_orders_section( $variation_group ) {
		...
		$group  = $parent->add_group(
			array(
				'id'         => 'woocommerce-pre-order-manager-group-tab',
				'attributes' => array(
					'title' => __( 'Pre-orders', 'nextxen-pre-order-manager' ),
				),
			)
		);
		...
		$section->add_block(
			array(
				'id'             => 'npom_active_pre_orders_message',
				'blockName'      => 'woocommerce-pre-order-manager/message-control-block',
				'attributes'     => array(
					'content' => sprintf(
						...
						esc_url( admin_url( 'admin.php?page=npom_manager&tab=actions&section=change-date&action_default_product=postIdPlaceholder' ) )
					),
				),
		...
		$section->add_block(
			array(
				'id'             => 'npom_change_settings_message',
				'blockName'      => 'woocommerce-pre-order-manager/message-control-block',
				'attributes'     => array(
					'content' => sprintf(
		...
		$section->add_block(
			array(
				...
				'attributes'        => array(
					'title'        => __( 'Enable pre-orders', 'nextxen-pre-order-manager' ),
					'label'        => sprintf(
						__( 'Enable pre-orders for this product.', 'nextxen-pre-order-manager' ),
					),
		...
		$section->add_block(
			array(
				...
				'attributes'        => array(
					'property' => 'meta_data._npom_fee',
					'label'    => __( 'Pre-order Fee', 'nextxen-pre-order-manager' ),
					'help'     => __( 'Set a fee to be charged when a pre-order is placed. Leave blank to not charge a pre-order fee.', 'nextxen-pre-order-manager' ),
				),
				'disableConditions' => array(
					array(
						'expression' => 'editedProduct.has_active_orders',
		...
		$section->add_block(
			array(
				'id'                => 'npom_availability_datetime',
				'blockName'         => 'woocommerce-pre-order-manager/date-time-picker',
				'attributes'        => array(
					'property' => 'availability_datetime',
					'title'    => __( 'Availability date/time', 'nextxen-pre-order-manager' ),
					'help'     => __( 'Set the date & time that this pre-order will be available. The product will behave as a normal product when this date/time is reached.', 'nextxen-pre-order-manager' ),
				),
				'disableConditions' => array(
					array(
						'expression' => 'editedProduct.has_active_orders',
		...
		$section->add_block(
			array(
				'id'                => 'npom_when_to_charge',
				'blockName'         => 'woocommerce-pre-order-manager/select-control-block',
				'attributes'        => array(
					'property' => 'meta_data._npom_when_to_charge',
					'title'    => __( 'When to Charge', 'nextxen-pre-order-manager' ),
					'help'     => __( 'Select "Upon Release" to charge the entire pre-order amount (the product price + pre-order fee if applicable) when the pre-order becomes available. Select "Upfront" to charge the pre-order amount during the initial checkout.', 'nextxen-pre-order-manager' ),
					'options'  => array(
						array(
							'value' => 'upfront',
							'label' => __( 'Upfront', 'nextxen-pre-order-manager' ),
						),
						array(
							'value' => 'upon_release',
							'label' => __( 'Upon Release', 'nextxen-pre-order-manager' ),
						),
					),
				),
				'disableConditions' => array(
...
new NPOM_Product_Editor_Compatibility();
