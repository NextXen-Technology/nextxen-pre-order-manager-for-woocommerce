...
class NPOM_Admin_Orders {
	...
	public function __construct() {
		...
		if ( NPOM::is_hpos_enabled() && is_admin() ) {
	...
	public function maybe_allow_resend_of_pre_order_emails( $available_emails ) {
		...
		if ( NPOM_Order::order_contains_pre_order( $theorder ) ) {

			$emails[] = 'npom_pre_ordered';

			$pre_order_status = NPOM_Order::get_pre_order_status( $theorder );

			if ( 'cancelled' === $pre_order_status ) {
				$emails[] = 'npom_pre_order_cancelled';
			}

			if ( 'completed' === $pre_order_status ) {
				$emails[] = 'npom_pre_order_available';
			}
		}
		...
		foreach ( $mails as $mail ) {
			if ( in_array( $mail->id, $emails ) && 'no' !== $mail->enabled ) {
				...
				$new_emails[ 'send_email_' . esc_attr( $mail->id ) ] = sprintf( __( 'Resend %s', 'nextxen-pre-order-manager' ), esc_html( $mail->title ) );
			}
		}
	...
	public function maybe_set_pre_order_for_pre_order_products( $order_id ) {
		...
		foreach ( $order->get_items( 'line_item' ) as $item ) {
			...
			if ( NPOM_Product::product_can_be_pre_ordered( $product ) ) {
				...
				$order->update_meta_data( '_npom_when_charged', get_post_meta( $product->get_id(), '_npom_when_to_charge', true ) );
				$order->save();
	...
	public function restrict_manage_pre_order( $order_type = '' ) {
		...
		<select name='shop_order_subtype' id='dropdown_shop_order_pre_order_type'>
			<option value=""><?php esc_html_e( 'All orders types', 'nextxen-pre-order-manager' ); ?></option>
			<?php
			$order_types = array(
				'non-pre-orders'  => _x( 'Non Pre-Orders', 'An order type', 'nextxen-pre-order-manager' ),
				'pre-orders-only' => _x( 'Pre-Orders Only', 'An order type', 'nextxen-pre-order-manager' ),
			);
	...
	public function add_pre_order_filter( $order_types ) {
		$order_types['non-pre-orders']  = _x( 'Non Pre-Orders', 'An order type', 'nextxen-pre-order-manager' );
		$order_types['pre-orders-only'] = _x( 'Pre-Orders Only', 'An order type', 'nextxen-pre-order-manager' );
	 ...
	 *
	 * Orders that have _npom_is_pre_order meta are considered a pre order
	 * Orders that DO NOT have _npom_is_pre_order meta are NOT considered a pre order
	 *
	 * @since 1.6.0
