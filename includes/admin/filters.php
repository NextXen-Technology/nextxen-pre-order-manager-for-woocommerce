...
add_filter(
	...
	/**
	 * Should return date in mysql date format for "_npom_availability_datetime" meta key.
	 *
	 * @since 2.0.0
	 *
	...
	function ( $meta_value, $meta ) {
		if ( $meta_value && '_npom_availability_datetime' === $meta->key ) {
			$meta_value = gmdate( 'Y-m-d H:i:s', $meta_value );
		}
...
add_filter(
	...
	/**
	 * Should return date in timestamp date format for "_npom_availability_datetime" meta key.
	 *
	 * @since 2.0.0
	 *
	...
	function ( $data ) {
		...
		$index     = array_search( '_npom_availability_datetime', $meta_keys, true );
