...
class NPOM_Manager {
	 ...
	 * @var string
	 */
	private static $scheduled_batch_processing_hook = 'npom_process_batch';
	 ...
	 * @var string
	 */
	private static $scheduled_pre_order_complete_hook = 'npom_complete_pre_order';
	...
	public function __construct() {
		...
		$disable = get_option( 'npom_disable_auto_processing', 'no' );

		if ( 'yes' !== $disable ) {
			add_action( 'npom_completion_check', array( $this, 'maybe_schedule_batch_processor' ) );
	...
	public function maybe_schedule_batch_processor() {
		$product_ids = NPOM_Product::get_products_passed_release_date();
	...
	public function schedule_actions_to_complete_pre_orders( $product_ids, $batch_id ) {
		...
		$batch_size    = apply_filters( 'npom_complete_pre_orders_batch_size', 200 );
	...
	public function maybe_prevent_pending_order_cancel( $cancel_order, $order ) {
		if ( NPOM_Order::order_contains_pre_order( $order ) && 'completed' === NPOM_Order::get_pre_order_status( $order ) ) {
			$cancel_order = false;
		}
	...
	public function maybe_prevent_payment_complete_order_stock_reduction( $reduce_stock, $order_id ) {
		...
		if ( NPOM_Order::order_contains_pre_order( $order ) && NPOM_Order::order_will_be_charged_upon_release( $order ) && ! self::is_order_pay_later( $order_id ) ) {
			$reduce_stock = false;
		}
			...
			$order_ids = $wpdb->get_results(
				$wpdb->prepare(
					...
					WHERE
						...
						order_meta.meta_key = '_npom_is_pre_order' AND
			...
			$order_ids = $wpdb->get_results(
				$wpdb->prepare(
					...
					WHERE
						...
						item_meta.meta_key = '_product_id' AND
						item_meta.meta_value = %s AND
						post_meta.meta_key = '_npom_is_pre_order' AND
	 ...
	 *
	 * @since 1.0
	 * @since 2.2.9 Added $page_number and $query parameters.
	 *
	 * @param int            $user_id     Optional user id to return pre-orders for. Defaults to the currently logged in user.
	 * @param int            $page_number Optional page number to return pre-orders for. Defaults to 1.
	 * @param array|stdClass $query       Optional reference to a query object that will be populated with the query results. Passed by reference.
	 * @return array of WC_Order objects
	 */
	public static function get_users_pre_orders( $user_id = null, $page_number = 1, &$query = null ) {
		...
		return apply_filters( 'npom_users_pre_orders', $orders, $user_id );
	}
	...
	public static function can_pre_order_be_changed_to( $new_status, $order ) {
		...
		$status = NPOM_Order::get_pre_order_status( $order );
		...
		return apply_filters( 'npom_status_can_be_changed_to_' . $new_status, $can_be_changed, $order );
	}
	...
	public static function get_users_change_status_link( $new_status, $order ) {
		...
		return apply_filters( 'npom_users_action_link', $action_link, $order, $new_status );
	}
	...
	/**
	 * Checks if there are any pre-order enabled products
	 *
	 * @since 2.3.2
	 *
	 * @return bool true if there are any pre-order enabled products, false otherwise
	 */
	public static function has_pre_order_enabled_products() {
		$args = array(
			'post_type'              => 'product',
			'post_status'            => array( 'publish', 'private' ),
			'numberposts'            => 1,
			...
			'meta_key'               => '_npom_enabled', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
			'meta_value'             => 'yes', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_value
		);

		$product_post_ids = get_posts( $args );

		return ! empty( $product_post_ids );
	}
	...
	public static function email_pre_order_customer( $order, $message ) {
		...
		if ( 'active' !== NPOM_Order::get_pre_order_status( $order ) ) {
			return;
		}
	...
	public static function change_release_date_for_all_pre_orders( $product, $new_availability_date, $message = '' ) {
		...
		try {
			...
			global $npom_manager;
			$npom_manager->log( $e->getMessage() );
			$timestamp = '';
		}

		// set new availability date for product
		update_post_meta( $product->get_id(), '_npom_availability_datetime', $timestamp );
		$product_id = $product->get_id();
		$is_enabled = ! $is_past_date && $timestamp && NPOM_Product::product_can_be_pre_ordered( $product_id );

		/** This hook is documented in includes/admin/class-npom-admin-products.php */
		do_action( 'npom_after_product_options_updated', $product_id, $is_enabled, $timestamp );
		...
		foreach ( $orders as $order ) {
			...
			if ( $is_past_date && 'pre-ordered' === $order->get_status() ) {
				// Doing this now to not to wait for the 'npom_completion_check' cron job.
				self::complete_pre_order( $order );
			}

			// Add 'release date changed' order note for admins.
			/* translators: %s: Availability date */
			$order->add_order_note( sprintf( __( 'Pre-order release date changed to %s', 'nextxen-pre-order-manager' ), NPOM_Product::get_localized_availability_date( $product, __( 'N/A', 'nextxen-pre-order-manager' ) ) ) );
		...
		if ( $is_past_date ) {
			...
			update_post_meta( $product->get_id(), '_npom_enabled', 'no' );

			do_action( 'npom_pre_orders_disabled_for_product', $product->get_id() );
		}
	}
	...
	public static function is_order_pay_later( $order_id = null ) {
		...
		$order = wc_get_order( $order_id );
		return is_object( $order ) && 'yes' === $order->get_meta( '_npom_is_pay_later', true );
	}
	...
	public static function complete_pre_order( $order, $message = '' ) {
		...
		add_filter( 'npom_processing_order_completion', '__return_true' );
		...
		// See https://github.com/woocommerce/woocommerce-pre-order-manager/issues/345
		$transient_key = 'npom_pre_order_completed_message_' . $order->get_id();
		if ( ! empty( $message ) ) {
		...
		if ( NPOM_Order::order_will_be_charged_upon_release( $order ) ) {
			...
			if ( ! $zero_cost_order ) {
				...
				if ( NPOM_Order::order_has_payment_token( $order ) || self::is_order_pay_later( $order_id ) ) {
					// load payment gateways.
					WC()->payment_gateways();

					// fire action for payment gateway to charge pre-order.
					do_action( 'npom_process_pre_order_completion_payment_' . $order->get_payment_method(), $order );
				}
			} else {
		...
		NPOM_Order::update_pre_order_status( $order, 'completed', $message );

		if ( ! doing_action( self::$scheduled_pre_order_complete_hook ) ) {
			/*
			 * Manual completion, remove any scheduled completion of the order.
			 *
			 * Any scheduled tasks to complete the order are removed to prevent any errors
			 * caused by attempting to complete a completed order.
			 */
			WC()->queue()->cancel( self::$scheduled_pre_order_complete_hook, array( 'order_id' => $order->get_id() ) );
		}

		do_action( 'npom_pre_order_completed', $order, $message );
		remove_filter( 'npom_processing_order_completion', '__return_true' );
	}
	...
	public static function cancel_pre_order( $order, $message = '' ) {
		...
		NPOM_Order::update_pre_order_status( $order, 'cancelled', $message );

		// add 'cancelled' order note for admins
		$order->add_order_note( __( 'Pre-order cancelled', 'nextxen-pre-order-manager' ) );

		// update the order status
		$order->update_status( 'cancelled' );

		do_action( 'npom_pre_order_cancelled', $order, $message );

		/*
		 * Remove any scheduled completion of the order.
		 *
		 * Any scheduled tasks to complete the order are removed to prevent any errors
		 * caused by attempting to complete a cancelled order.
		 */
		try {
			WC()->queue()->cancel( self::$scheduled_pre_order_complete_hook, array( 'order_id' => $order->get_id() ) );
		} catch ( Exception $e ) {
			 ...
			 * Do nothing.
			 *
			 * The caught error indicates that the order was not scheduled for completion. This is
			 * not an error in the context of this code.
			 */
			unset( $e ); // Keep PHPCS happy.
		}
	}
	 ...
	 *
	 * @see NPOM_Manager::cancel_pre_orders()
	 *
	 * @since 1.0
	 * @param object|int $product the product to complete all pre-orders for
	 * @param string $message an optional message to include in communications to the customer
	...
	public static function get_formatted_pre_order_total( $total, $product ) {
		...
		if ( NPOM_Product::product_is_charged_upon_release( $product ) ) {
			$formatted_total = get_option( 'npom_upon_release_order_total_format' );
		} else {
			$formatted_total = get_option( 'npom_upfront_order_total_format' );
		}
		...
		$formatted_total = str_replace( '{availability_date}', NPOM_Product::get_localized_availability_date( $product ), $formatted_total );

		// add order total
		$formatted_total = str_replace( '{order_total}', $total, $formatted_total );

		return apply_filters( 'npom_pre_order_order_total', $formatted_total, $product );
	}
	...
	private function disable_pre_orders_for_products( $product_ids ) {
		...
		foreach ( $product_ids as $product_id ) {
			update_post_meta( $product_id, '_npom_enabled', 'no' );

			do_action( 'npom_pre_orders_disabled_for_product', $product_id );
		}
	}
	...
	public function check_cancel_pre_order() {
		...
		$string = __( 'Your pre-order has been cancelled.', 'nextxen-pre-order-manager' );
	...
	public function maybe_activate_preorder( $product_id, $stock_status, $product ) {
		...
		if ( 'product' !== $typenow && 'outofstock' === $stock_status && 'yes' === get_option( 'npom_auto_pre_order_out_of_stock' ) ) {
			if ( ! NPOM_Product::product_supports_pre_order_feature( $product ) ) {
				// Product type/feature set does not support pre-orders.
				return;
			}
			...
			update_post_meta( $product->get_id(), '_npom_enabled', 'yes' );
			$product->set_manage_stock( false );
			$product->set_stock_status( 'instock' );
			$product->save();
		}
	}
} // end \NPOM_Manager class
