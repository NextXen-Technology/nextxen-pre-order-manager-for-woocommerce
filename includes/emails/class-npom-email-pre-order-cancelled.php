 ...
 * @since 1.0
 */
class NPOM_Email_Pre_Order_Cancelled extends WC_Email {
	...
	public function __construct() {
		...
		$this->id          = 'npom_pre_order_cancelled';
		$this->title       = __( 'Pre-order cancelled', 'nextxen-pre-order-manager' );
		$this->description = __( 'This is an order notification sent to the customer after a pre-order is cancelled.', 'nextxen-pre-order-manager' );

		$this->heading = __( 'Pre-order cancelled', 'nextxen-pre-order-manager' );
		$this->subject = __( 'Your {site_title} pre-order from {order_date} has been cancelled', 'nextxen-pre-order-manager' );
